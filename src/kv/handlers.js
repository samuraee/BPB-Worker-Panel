...
export async function getDataset(request, env) {
    ...
    try {
        proxySettings = await env.genie.get("proxySettings", {type: 'json'});
        warpConfigs = await env.genie.get('warpConfigs', {type: 'json'});
    } catch (error) {
        console.log(error);
...
export async function updateDataset (request, env) {
    ...
    if (!isReset) {
        try {
            currentSettings = await env.genie.get("proxySettings", {type: 'json'});
        } catch (error) {
            console.log(error);
    ...
    try {    
        await env.genie.put("proxySettings", JSON.stringify(proxySettings));
